...
<body class="mt-1 patient-demographic bg-light">
    ...
    <div id="container_div" class="<?php echo $oemr_ui->oeContainer(); ?> mb-2">
        ...
        $getAbsIntakeFormDetails = sqlQuery("select abs.*,ic.name from abs_form_data abs left join insurance_companies as ic on ic.id = abs.insurance_name where abs.pid = ?", [$pid]);
        $refSource = json_decode($getAbsIntakeFormDetails['referral_source'], true);
        $refSourceArr = ['self_referral' => 'Self Referral', 'employee_assistant_program' => 'Employee Assistant Program', 'criminal_court' => 'Criminal Court', 'supreme_court' => 'Supreme Court', 'family_court' => 'Family Court', 'attorney' => 'Attorney', 'probation_parole_officer' => 'Probation / Parole Officer', 'social_worker' => 'Social Worker', 'case_manager' => 'Case Manager', 'child_welfare_worker' => 'Child Welfare Worker/ACS', 'doctor' => 'Doctor'];
        $refSr = [];
        foreach ($refSource as $key => $value) {
            $refSr[] = $refSourceArr[$value];
        }

        $pgmSource = json_decode($getAbsIntakeFormDetails['treatment_name'], true);
        $pgmSourceArr = ['general_mental_health' => 'General Mental Health', 'anger_management_stabilization_program' => 'Anger Management & Stabilization Program', 'violence_prevention' => "Violence Prevention and Intervention Program/Domestic Violence/Batterers' Intervention Program", 'sexual_behavior_program' => 'Sexual Behavior Program', 'drinking_driving_program' => 'Drinking Driving Counseling Program'];
        $pgmSr = [];
        foreach ($pgmSource as $key => $value) {
            $pgmSr[] = $pgmSourceArr[$value];
        }

        $intakeForm = '';
        $intakeForm .= '<div>';
        $intakeForm .= '<section class="card mb-2">
    <div class="card-body p-1">
        <h6 class="card-title mb-0 d-flex p-1 justify-content-between">
            <a class="text-left font-weight-bolder collapsed" href="#" data-toggle="collapse" data-target="#intake_ps_expand" aria-expanded="false" aria-controls="intake_ps_expand">Intake Appointment Information</a>
                    </h6>
	<div id="intake_ps_expand" class="card-text collapse" style="">
 <div class = "row">
                        <div class = "col-sm-6"><b>What programs would you like to attend : </b></div>
                        <div class = "col-sm-6">' . implode(', ', $pgmSr) . '</div>
                </div>
		<div class = "row">
                        <div class = "col-sm-6"><b>Who is referring you to the program : </b></div>
                        <div class = "col-sm-6">' . implode(',', $refSr) . '</div>
                </div>
		<div class = "row">
			<div class = "col-sm-6"><b>Name of the referral agency and or contact person : </b></div>
			<div class = "col-sm-6">' . $getAbsIntakeFormDetails['referral_source_name'] . '</div>
		</div>
		<div class = "row">
			<div class = "col-sm-6"><b>The phone number of the referral contact person : </b></div>
			<div class = "col-sm-6">' . $getAbsIntakeFormDetails['referral_source_phone'] . '</div>
		</div>
		<div class = "row">
			<div class = "col-sm-6"><b>The email address of the referral contact person : </b></div>
			<div class = "col-sm-6">' . $getAbsIntakeFormDetails['referral_source_email'] . '</div>
		</div>
		<div class = "row">
                        <div class = "col-sm-6"><b>The insurance Company name : </b></div>
                        <div class = "col-sm-6">' . $getAbsIntakeFormDetails['name'] . '</div>
		</div>
		<div class = "row">
    ...
    <div class="card-body p-1">
        <h6 class="card-title mb-0 d-flex p-1 justify-content-between">
            <a class="text-left font-weight-bolder collapsed" href="#" data-toggle="collapse" data-target="#progressNote_ps_expand" aria-expanded="false" aria-controls="progressNote_ps_expand">Progress / Encounter Notes</a>
                    </h6>
        <div id="progressNote_ps_expand" class="card-text collapse" style="">
		 <div class = "">';
        $getEncNotes = sqlStatement("select DATE_FORMAT(f.date, '%Y-%m-%d') AS date, f.reason, f.encounter,f.facility,oc.pc_catname from form_encounter f left join openemr_postcalendar_categories oc on oc.pc_catid = f.pc_catid where f.pid = ? order by date desc", [$pid]);
        $progressNote .= '<table class = "table" ><tr style = "color:#ce3e29"><th>Encounter Id</th><th>Encounter Date</th><th>Visit category</th><th>Facility</th><th>Reason</th></tr>';
        while ($row = sqlFetchArray($getEncNotes)) {
            $progressNote .= '<tr><td>' . $row['encounter'] . '</td><td>' . $row['date'] . '</td><td>' . $row['pc_catname'] . '</td><td>' . $row['facility'] . '</td><td>' . $row['reason'] . '</td></tr>';
        }
        $progressNote .= '</table>';
        $progressNote .= '</div>
	</div>
        ...
        $progressReport = '';
        $progressReport .= '<div>';
        $progressReport .= '<section class="card mb-2  ">
    <div class="card-body p-1">
        <h6 class="card-title mb-0 d-flex p-1 justify-content-between">
            <a class="text-left font-weight-bolder collapsed" href="#" data-toggle="collapse" data-target="#progressReport_ps_expand" aria-expanded="false" aria-controls="progressReport_ps_expand">Progress Report</a>
                    </h6>
        <div id="progressReport_ps_expand" class="card-text collapse" style="">
                 <div class = "row">
                 </div>
        </div>
     </div>
